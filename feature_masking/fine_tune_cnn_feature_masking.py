...
from layers.fm_net_convnext import FeatureMaskingNet as FeatureMaskingNetLastStage
from layers.fm_second_last_stage_convnext import FeatureMaskingNetSecondLastStage
...
def parse_args():
    ...
    parser.add_argument('--model_arch', default='resnet50', type=str, help='pick model architecture')
    ...
    # Feature masking params
    parser.add_argument('--feature_masking_last_layer', action='store_true', default=False)
...
def main():
    ...
    if 'convnext' in args.model_arch:
        torch.nn.init.kaiming_normal_(model_base.head.fc.weight, mode='fan_out', nonlinearity='relu')
        torch.nn.init.zeros_(model_base.head.fc.bias)
    ...
    if args.freeze_backbone:
        if 'convnext' in args.model_arch:
            ...
            if not args.feature_masking_last_layer:
                for param in model_base.stages[-1].parameters():
                    param.requires_grad = True
                for param in model_base.norm_pre.parameters():
                    param.requires_grad = True

        else:
            raise NotImplementedError('Freezing backbone is only implemented for ConvNeXt models.')
    ...
    # Start the timer
    start_time = timer()
    if args.feature_masking_last_layer:
        model_fm = FeatureMaskingNetLastStage(model_base, model_seg)
    else:
        model_fm = FeatureMaskingNetSecondLastStage(model_base, model_seg)
